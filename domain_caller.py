...
def chr_split(inputfile):
    """Takes bedgraph and outputs dataframe"""
...
def domain_caller(lyst, outputfile, win_size=100):
    """Meat and Potatoes"""
    if win_size < 5:
    ...
    loop=1
    look_thru = 3 #this variable will allow stocastic negative values to be skipped
    for i in range(0,len(lyst),1):
        ...
        if (loop == 0 and float(lyst[i][3]) <0):
            diff_item=-1
            diff_holder=[]
            while diff_item < look_thru:
                diff_item+=1
                try:
                    diff_holder.append(float(lyst[i+diff_item][3]))
                except IndexError:
                    continue #might want to use Pass here
            if (sum(n<0 for n in diff_holder)) <= threshold:
                continue
            elif (sum(n<0 for n in diff_holder)) > threshold:
                end=lyst[i-1][2]
